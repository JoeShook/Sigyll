...
public class CertificateManagementService
{
    ...
    public static async Task<int?> FindCaBySkiInternalAsync(
        SigyllDbContext db, int trustDomainId, string authorityKeyIdentifier, CancellationToken ct = default)
    {
    ...
    public static async Task<int?> FindCaByDnAndSignatureInternalAsync(
        SigyllDbContext db, int trustDomainId, X509Certificate2 cert, CancellationToken ct = default)
    {
        var cas = await db.CaCertificates
            .Where(ca => ca.TrustDomainId == trustDomainId)
            .ToListAsync(ct);
    ...
    private static async Task<int?> FindMatchingCaInTrustDomainAsync(
        SigyllDbContext db, string? certPem, int targetTrustDomainId, CancellationToken ct)
    {
