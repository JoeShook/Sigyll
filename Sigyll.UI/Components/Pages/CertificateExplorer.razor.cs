...
namespace Sigyll.UI.Components.Pages;
...
public partial class CertificateExplorer : IDisposable
{
    ...
    [Inject] private IDbContextFactory<SigyllDbContext> DbFactory { get; set; } = null!;
    ...
    private Task<List<(string Url, string Error)>> ValidateEndpointUrlsAsync(List<string> cdpUrls, List<string> aiaUrls)
    {
        ...
        // these URLs are consumed by clients at runtime, not by Sigyll's host, so resolvability
        // from Sigyll's network is irrelevant and the HEAD probe just slows things down.
